# CLAUDE.md — FloatNote（夢日記＋メモアプリ）

## プロジェクト概要

アプリ名は **FloatNote**（日本語表記：フロートノート）。「浮かぶ・漂う断片を書き留める」の意。
夢日記から始まったが、夢だけでなく**アイデアや日々の気づき**も記録できるメモアプリ。
朝目覚めた直後（または夜中）に眠い目をこすりながら、消えやすい記憶や思いつきを書き留められる。
カレンダーUIで「いつ・何を書いたか」を一目で把握できるシンプルな日記アプリ。

| 項目 | 内容 |
|------|------|
| 使用者 | 自分を含む少数の限定ユーザー（各自のアカウントで完全に分離） |
| 主な目的 | 実際に使えるプロダクトを作る |
| 開発方針 | **MVP（最小限で動くもの）を最速で完成させる** |
| 使用シーン | 朝起きた直後・夜中、ベッドの中で片手スマホ操作 |

> **現在のアクセス制限の正体（混乱しやすいので明記）**：データ分離は Supabase の RLS で既に
> 何人でも対応可能。今ごく少数に限定しているのは **Google OAuth 同意画面が「テスト」公開
> ステータスで、登録済みの限定ユーザーしか認証を通れない**ためだけ（コード/Supabase に
> メール許可リストは無い）。一般公開（マルチユーザー化）の作業フローは Issue #18 で計画。
...
**実装する機能：**
- [x] カレンダーUI（月表示・スマホ最適化）
- [x] 前月・翌月のナビゲーション
- [x] 日付クリック → モーダルで**その日のエントリー一覧 + 「新規追加」ボタン**を表示
- [x] エントリー作成・編集・削除（タイトル + 本文）
- [x] **1日に複数エントリーを記録可能**（一晩で複数の夢を見たケース対応）
- [x] 記録済みの日付にドット（●）をつける（エントリー数に応じて色を濃くするなどの拡張も可）
...
- [x] Web Speech API で音声入力（マイクボタン）
  ...
  - `continuous=true` ＋ 自前の無音タイマー（既定2秒）：小休止では切らず、2秒沈黙したら自動停止。
    連続自動リスタート（onend で勝手に start し直す）はしない＝iOS で不安定なため
  - **クイック音声 FAB**：カレンダー下の余白にマイクボタン。タップで「今日」の新規フォームを開き、
    その場で録音を自動開始（`autoStart`）。手入力経路（日付タップ→新規追加／編集）では自動開始しない
  - 非対応ブラウザはマイクボタン・FAB を非表示（グレースフル）。ロジックは `useSpeechRecognition.js` に分離
  - **要HTTPS**：Web Speech は secure context 必須。実機検証は Vercel(HTTPS) で行う（LAN の http://IP では使えない）
  - 既知の注意：FAB の自動開始はブラウザのジェスチャー要件に依存。iPhone Safari では弾かれる可能性があり、
    その場合もフォーム内「音声」ボタンで手動開始できる（要実機確認）
...
> **デザイン哲学：原研哉（白／間／引き算／静けさ）。** 太く目立たせるより、余白と字間で呼吸させる。
> 線や色は足すより引く。「何もないが、すべてを受け止める器」を目指す。下表はその具体方針。
